...
class DeclarativeBase(
    ...
    ClassStream-level attributes which may be specified include:
    ...
    * :meth:`_orm.registry.map_imperatively` will produce a
    ...
    def _add_manager(self, manager: ClassManager[Any]) -> None:
        ...
        if manager.is_mapped:
            raise exc.ArgumentError(
                "ClassStream '%s' already has a primary mapper defined. "
                % manager.class_
            )
    ...
    def mapped_as_dataclass(
        ...
        *,
        ...
        """ClassStream decorator that will apply the Declarative mapping process
    ...
    def as_declarative_base(self, **kw: Any) -> Callable[[Type[_T]], Type[_T]]:
        """
        ClassStream decorator which will invoke
...
def as_declarative(**kw: Any) -> Callable[[Type[_T]], Type[_T]]:
    """
    ClassStream decorator which will adapt a given class into a
    :func:`_orm.declarative_base`.
