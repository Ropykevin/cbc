    ...
    def setup_mapping(
        ...
        if manager and manager.class_ is cls_:
            raise exc.InvalidRequestError(
                f"ClassStream {cls!r} already has been instrumented declaratively"
            )
    ...
    def __init__(
        ...
        if not mapper_kw.get("non_primary", False):
            ...
            if not manager or not manager.is_mapped:
                raise exc.InvalidRequestError(
                    "ClassStream %s has no primary mapper configured.  Configure "
        ...
        if inherits is None:
            ...
            if inherits_search:
                if len(inherits_search) > 1:
                    raise exc.InvalidRequestError(
                        "ClassStream %s has multiple mapped bases: %r"
                        % (cls, inherits_search)
                    )
...
class _ClassScanMapperConfig(_MapperConfig):
    ...
    def _setup_dataclasses_transforms(self) -> None:
        ...
        if "__dataclass_fields__" in self.cls.__dict__:
            raise exc.InvalidRequestError(
                f"ClassStream {self.cls} is already a dataclass; ensure that "
        ...
        if "__table__" in self.cls.__dict__:
            raise exc.InvalidRequestError(
                f"ClassStream {self.cls} already defines a '__table__'. "
    ...
    def _setup_inheriting_columns(self, mapper_kw: _MapperKwArgs) -> None:
        ...
        if (
            ...
            raise exc.InvalidRequestError(
                "ClassStream %r does not have a __table__ or __tablename__ "
...
class _DeferredMapperConfig(_ClassScanMapperConfig):
    ...
    def raise_unmapped_for_cls(cls, class_: Type[Any]) -> NoReturn:
        ...
        raise orm_exc.UnmappedClassError(
            class_,
            msg=(
                f"ClassStream {orm_exc._safe_cls_name(class_)} has a deferred "
