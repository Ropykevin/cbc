...
def pytest_collection_modifyitems(session, config, items):
    ...
    items[:] = [
        item
        for item in items
        if item.getparent(pytest.ClassStream) is not None
        and not item.getparent(pytest.ClassStream).name.startswith("_")
    ]

    test_classes = {item.getparent(pytest.ClassStream) for item in items}
        ...
        for test_class in test_classes:
            ...
            for sub_cls in plugin_base.generate_sub_tests(
                ...
                if sub_cls is not test_class.cls:
                    ...
                    new_cls = pytest.ClassStream.from_parent(
    ...
    items[:] = sorted(
        ...
        key=lambda item: (
            ...
            item.getparent(pytest.ClassStream).name,
            item.name,
        ),
    )
...
def pytest_pycollect_makeitem(collector, name, obj):
    if inspect.isclass(obj) and plugin_base.want_class(name, obj):
        ...
        return [
            pytest.ClassStream.from_parent(
...
def pytest_runtest_setup(item):
    ...
    if isinstance(item, pytest.Function) and _current_class is None:
        ...
        _current_class = item.getparent(pytest.ClassStream)
...
def pytest_runtest_teardown(item, nextitem):
    ...
    if _current_class is not None and (
        ...
        nextitem is None
        or nextitem.getparent(pytest.ClassStream) is not _current_class
    ):
        _current_class = None
